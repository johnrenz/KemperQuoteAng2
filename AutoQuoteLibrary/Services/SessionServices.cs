...
namespace AutoQuoteLibrary.Services
{
    ...
    {
        public WebSession Load(ref Guid guid, string zip, Boolean initalize)
        {
            try
            {
                ...
                if ((guid != null) && (guid != Guid.Empty))
                    request = new XElement("Request", new XElement("Guid", guid.ToString()), new XElement("ZipCode", zip), new XElement("ClickThruPartnerInfo", new XElement("CTID", "10160"), new XElement("Custom", ""), new XElement("MarketKeyCode", ""), new XElement("SalesPhone", ""), new XElement("SalesHours", ""), new XElement("AMFAccountNumber", ""), new XElement("LandingPage", ""), new XElement("HTTPReferrer", ""), new XElement("Affinity", new XElement("IsAffinity", ""), new XElement("IsAgent", ""), new XElement("Logo", ""), new XElement("Description", ""))));
                else
                {
                    request = new XElement("Request", new XElement("ZipCode", zip), new XElement("ClickThruPartnerInfo", new XElement("CTID", "10160"), new XElement("Custom", ""), new XElement("MarketKeyCode", ""), new XElement("SalesPhone", ""), new XElement("SalesHours", ""), new XElement("AMFAccountNumber", ""), new XElement("LandingPage", ""), new XElement("HTTPReferrer", ""), new XElement("Affinity", new XElement("IsAffinity", ""), new XElement("IsAgent", ""), new XElement("Logo", ""), new XElement("Description", ""))));
                }
                ...
                if (response.Element("Coverages") != null)
                {
                    if (response.Element("Coverages").Element("PolicyCoverages") != null)
                    {
                        ...
                        foreach (XElement covElement in response.Element("Coverages").Element("PolicyCoverages").Elements())
                        {
                            ...
                            else
                            {
                                ...
                                foreach (XElement limitElement in covElement.Element("Limits").Elements("Limit"))
                                {
                                    ...
                                    if (limitElement.GetValue("Value") == cov.SelectedValue)
                                    {
                                        ...
                                        {
                                            case "PIPFPB":
                                                cov.SelectedLimit.Caption = limitElement.GetValue("Caption").formatFPB();
                                                break;
                                            case "PIPCombined":
                                                cov.SelectedLimit.Caption = limitElement.GetValue("Caption").formatPIPCombined();
                                                break;
                                            default:
                                                cov.SelectedLimit.Caption = limitElement.GetValue("Caption");
                                                break;
                                        }
                                    }
                                }
                            ...
                            cov.Caption = (covElement.GetValue("Desc"));

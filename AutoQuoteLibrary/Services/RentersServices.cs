...
namespace KdQuoteLibrary.Services
{
    public class RentersServices : BaseServices
    {
        ...
        public XElement LoadCoverages(WebSession session)
        {
            ...
            if (scenarios == null)
            {
                Scenarios ws = new Scenarios { Url = Environmentals.HOWebService + "/DecisionMaker/quotesvc/Scenarios.asmx" };
        ...
        public Int32 LoadAccount(String keycode)
        {
            ...
            if (account == null)
            {
                AccountMaster ws = new AccountMaster { Url = Environmentals.HOWebService + "/DecisionMaker/aspscript/AccountMaster.asmx" };
